...
(function() {
  ...
  Coalmine = (function() {
    ...
    Coalmine.prototype.URL_BASE = ":scheme://:host/events?signature=:signature&json=:json";

    Coalmine.prototype.MSIE_MAX_URL_LENGTH = 2048;

    Coalmine.prototype.TRUNCATED_SEQ = "(...)";
    ...
    Coalmine.prototype.configure = function(callback) {
      return callback(this.config);
    };

    Coalmine.prototype.critical = function(message, additionalData) {
      additionalData.severity = "critical";
      return this.notify(message, additionalData);
    };

    Coalmine.prototype.error = function(message, additionalData) {
      additionalData.severity = "error";
      return this.notify(message, additionalData);
    };

    Coalmine.prototype.warning = function(message, additionalData) {
      additionalData.severity = "warning";
      return this.notify(message, additionalData);
    };

    Coalmine.prototype.info = function(message, additionalData) {
      additionalData.severity = "info";
      return this.notify(message, additionalData);
    };

    Coalmine.prototype.debug = function(message, additionalData) {
      additionalData.severity = "debug";
      return this.notify(message, additionalData);
    };

    Coalmine.prototype.setCustomVariable = function(key, value) {
      return this.config.customVariables[key] = value;
    };

    Coalmine.prototype.notify = function(message, additionalData) {
      var e, error, html, script, sendable, src, _ref;
      ...
      error = this.createNotification(message, additionalData);
      this.errors.push(error);
      sendable = (_ref = this.config.environment, __indexOf.call(this.config.enabledEnvironments, _ref) >= 0);
      if (sendable) {
        src = this.createSafeUrl(error);
        if (!src) {
          return;
        }
    ...
    Coalmine.prototype.createSafeUrl = function(error) {
      var excess, i, isMsie, longParameter, longParameters, newLength, parameter, totalLength, url, _i, _j, _len, _len1;

      url = this.createUrl(error);
      isMsie = error.user_agent.indexOf("MSIE") > -1;
      if (!((error.user_agent != null) && isMsie)) {
        return url;
      }
      excess = url.length - this.MSIE_MAX_URL_LENGTH;
      if (excess <= 0) {
        return url;
      }
      longParameters = [
      ...
      excess = url.length - this.MSIE_MAX_URL_LENGTH;
      totalLength = 0;
      for (i = _i = 0, _len = longParameters.length; _i < _len; i = ++_i) {
        longParameter = longParameters[i];
      ...
      if (totalLength >= excess) {
        longParameters.sort(function(a, b) {
          return b.weight - a.weight;
        });
        for (_j = 0, _len1 = longParameters.length; _j < _len1; _j++) {
          longParameter = longParameters[_j];
          excess = url.length - this.MSIE_MAX_URL_LENGTH;
          if (excess <= 0) {
            return url;
          }
          if (longParameter.weight <= 0) {
            continue;
          }
          parameter = error[longParameter.name];
          newLength = parameter.length - excess - this.TRUNCATED_SEQ.length;
          if (newLength < 0) {
            newLength = 0;
          }
          error[longParameter.name] = parameter.substring(0, newLength) + this.TRUNCATED_SEQ;
          url = this.createUrl(error);
        }
      }
      return url;
    ...
    Coalmine.prototype.createUrl = function(error) {
      var url;

      url = this.URL_BASE;
      url = url.replace(":scheme", this.config.scheme);
      url = url.replace(":host", this.config.host);
      url = url.replace(":signature", this.config.signature);
      url = url.replace(":json", encodeURIComponent(JSON.stringify(error)));
      return url;
    };

    Coalmine.prototype.createNotification = function(message, additionalData) {
      var key, line, notification, severity, url, version;
      ...
      notification = {
        ...
        application: this.config.customVariables,
        url: this.getRequestUrl(url),
        method: this.config.method,
        parameters: this.config.parameters || this.getParameters(url),
        ...
        stack_trace: this.getStackTrace(arguments),
        message: message || "",
        line_number: line || ""
      };
    ...
    Coalmine.prototype.getRequestUrl = function(url) {
    ...
    Coalmine.prototype.getParameters = function(url) {
      var anchor, parameters;

      url = this.getRequestUrl(url);
      parameters = "";
      if (url) {
        if (url.indexOf("?") !== -1) {
          parameters = url.replace(/.*\?/, "").replace(/#.*/, "").replace(/&$/, "");
        }
        if (url.indexOf("#") !== -1) {
          anchor = url.replace(/.*#/, "");
          if (/(?:.+=[^\&]*)+/.test(anchor)) {
    ...
    Coalmine.prototype.getStackTrace = function(message, url, line) {
      var stackTrace;
    ...
    Coalmine.prototype.getErrors = function() {
      return this.errors;
    };

    Coalmine.prototype.hasMessage = function(string) {
      var error, _i, _len, _ref;

      _ref = this.errors;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        error = _ref[_i];
        if (error.message.indexOf(string) !== -1) {
          return true;
        }
      }
      return false;
